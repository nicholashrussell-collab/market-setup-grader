...
values (
  ...
  false,
  100,
  'v9.7 week-ready tracked-symbol defaults. Broker-synced Paper Trading is the default route and starts disarmed.'
)
...
-- v9.7 tracked-symbol-only control panel. Safe to run multiple times.
update public.bot_control
set universe_label = 'Tracked Symbols',
    ...
    notes = coalesce(notes, 'v9.7 admin: tracked symbols list is the cloud bot watchlist.')
where id = 'main';

-- v9.7 paper-trading guardrails and best-bot profile controls. Safe to run multiple times.
...
alter table public.bot_control add column if not exists grade_profile text not null default 'Pullback';
alter table public.bot_control add column if not exists direction_filter text not null default 'Long';
alter table public.bot_control add column if not exists regime_filter text not null default 'Off';
alter table public.bot_control add column if not exists session_filter text not null default 'MiddayAfternoon';
alter table public.bot_control add column if not exists setup_type_filter text not null default 'AdaptiveBest';
alter table public.bot_control add column if not exists strategy_engine text not null default 'UniversalAdaptiveProV3';
alter table public.bot_control add column if not exists leader_exit_mode text not null default 'Fixed';
alter table public.bot_control add column if not exists risk_model text not null default 'Percent';
alter table public.bot_control add column if not exists fixed_risk_dollars numeric not null default 50;
alter table public.bot_control add column if not exists cooldown_minutes integer not null default 60;
alter table public.bot_control add column if not exists max_bars_to_hold integer not null default 120;
alter table public.bot_control add column if not exists warmup_bars integer not null default 200;
alter table public.bot_control add column if not exists max_new_trades_per_run integer not null default 1;
alter table public.bot_control add column if not exists max_total_open_risk_pct numeric not null default 8;
alter table public.bot_control add column if not exists account_type text not null default 'Cash';
alter table public.bot_control add column if not exists margin_multiplier numeric not null default 1;
alter table public.bot_control add column if not exists allow_fractional_shares boolean not null default true;
alter table public.bot_control add column if not exists allow_shorts boolean not null default false;
alter table public.bot_control add column if not exists open_start_minutes_et integer not null default 690;
alter table public.bot_control add column if not exists open_end_minutes_et integer not null default 960;
alter table public.bot_control add column if not exists no_new_trades_first_minutes integer not null default 0;
alter table public.bot_control add column if not exists no_new_trades_last_minutes integer not null default 0;
alter table public.bot_control add column if not exists no_overnight boolean not null default true;
...
set timeframe = '15Min',
    ...
    no_overnight = true,
    notes = coalesce(notes, 'v9.7 broker-only admin: 05/31 best profile, max one new trade per run, cooldown, broker health, and paper kill controls.')
where id = 'main';
