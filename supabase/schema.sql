...
values (
  ...
  false,
  100,
  'v9.1 week-ready tracked-symbol defaults. Paper execution starts disarmed.'
)
...
-- v9.1 tracked-symbol-only control panel. Safe to run multiple times.
update public.bot_control
set universe_label = 'Tracked Symbols',
    ...
    notes = coalesce(notes, 'v9.1 admin: tracked symbols list is the cloud bot watchlist.')
where id = 'main';

-- v9.1 paper-trading guardrails and best-bot profile controls. Safe to run multiple times.
...
set timeframe = '15Min',
    ...
    notes = coalesce(notes, 'v9.1 paper guardrails: 05/31 best profile, max one new trade per run, cooldown, broker health, and paper kill controls.')
where id = 'main';
